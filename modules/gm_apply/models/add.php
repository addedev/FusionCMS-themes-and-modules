...
class add extends CI_Model
{
	...
	public function accept()
	{
		...
		$queryst = $this->db->query("UPDATE gm_apply SET status = 'Accepted' where user_id = ?", array($this->input->post('applyid')));
		return $queryst;
	}
	...
	public function decline()
	{
		...
		$id = $this->db->query("SELECT id FROM account_data WHERE nickname = ?", array($userid));
		$id_row = $id->result_array();
		$queryst = $this->db->query("UPDATE gm_apply SET status = 'Declined' where user_id = ?", array($this->input->post('applyid')));
		return $queryst;
	}
}